...
Input: A JSON object representing an email, containing 'subject', 'body', 'sender', and an array of 'attachments' (each with 'filename' and 'local_path').
...
5. After upload completes, call create_document_record with a newly generated document_id (UUID string), the document_type, the returned blob_name from upload, the filename, and the email sender as sender_email.
6. Return a JSON array containing objects for each processed attachment:
   [{"document_id": "<uuid>", "blob_name": "<blob>", "filename": "<name>", "document_type": "<type>", "sender_email": "<sender_email>"}]
